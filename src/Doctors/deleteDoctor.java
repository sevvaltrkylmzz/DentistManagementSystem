...
public class deleteDoctor extends javax.swing.JFrame {
    ...
    public deleteDoctor() {
        ...
        Object columns[] = {"DID", "RoomNum", "FName", "MInit", "LastName", "Gender", "BirthDate","Email", "Address","Salary"};
    ...
    public void loadData() {
        ...
        String sql = "select DID,RoomNum,FName,MInit,LName,Gender,BirthDate,Email,Address,Salary from dentist";
        try {
            ps = connection.prepareStatement(sql);
            rs = ps.executeQuery();
            Object columnData[] = new Object[14];
            while (rs.next()) {
                columnData[0] = rs.getInt("DID");
                columnData[1] = rs.getString("RoomNum");
                columnData[2] = rs.getString("FName");
                columnData[3] = rs.getString("MInit");
    ...
    private void dDeletebtnMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_dDeletebtnMouseClicked
        connection = Connector.ConnectDb();
        String search = dDeleteField.getText();
        String sql = "Delete from dentist where DID ='" + search + "'";
        try {
            ps = connection.prepareStatement(sql);
            ps.execute();
            JOptionPane.showMessageDialog(null, "Dentist " + search + " has been deleted");
            defaultTableModel.getDataVector().removeAllElements();
            defaultTableModel.fireTableDataChanged();
            loadData();
            connection.close();
